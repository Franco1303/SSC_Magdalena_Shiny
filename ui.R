  ...
  tabsetPanel(id="main_tabs", type="tabs",
        ...
        del Ideam que monitorea el caudal de este corredor fluvial."
          ...
          incrementar la retrodispersión.",
              ...
              tabPanel("Modelo",
                       card(style_extra=paste0("border-left:4px solid ",COLOR_ACCENT,";",
                                               "background:linear-gradient(135deg,",COLOR_ACCENT,"10 0%,",COLOR_CARD," 55%);"),
                            section_title("Calibración y validación del modelo",
                                          "Comparación de modelos de regresión e índices de desempeño"),
                            div(style="display:flex;gap:12px;flex-wrap:wrap;margin-top:8px;",
                                uiOutput("model_header_chips"))
                       ),
                       card(
                         section_title("Configuración del modelo"),
                         div(style="display:flex;gap:24px;flex-wrap:wrap;align-items:flex-end;",
                             div(style="width:280px;",
                                 selectInput("model_selector","Modelo:",
                                             choices=setNames(names(MODEL_LABELS), MODEL_LABELS),
                                             selected="lineal")),
                             div(
                               radioButtons("model_y_var","Variable Y:",
                                            choices=c("SSC (mg/L)"="SSC"), selected="SSC", inline=TRUE))
                         ),
                         div(id="model_eq_box_wrap", style="margin-top:16px;",
                             uiOutput("model_eq_box"))
                       ),
                       uiOutput("model_metrics_row"),
                       div(style="display:grid;grid-template-columns:1fr 1fr;gap:20px;",
                           card(section_title("SSC medido vs modelado","Línea 1:1 ideal"),
                                plotlyOutput("model_1to1", height="400px")),
                           card(section_title("Residuos","SSC medido − SSC modelado"),
                                plotlyOutput("model_residuals", height="400px"))
                       ),
                       card(section_title("Desempeño por estación",
                                          "Ratio SSC_med / SSC_mod — rango aceptable ±20% (verde) y ±30% (amarillo)"),
                            plotlyOutput("model_ratio_km", height="440px")),
                       card(style_extra=paste0("border-left:4px solid ",ACCENT2,";"),
                            ...
                            uiOutput("loocv_metrics_row"),
                            div(style="display:grid;grid-template-columns:1fr 1fr;gap:20px;",
                                plotlyOutput("loocv_1to1", height="380px"),
                                plotlyOutput("loocv_error_dist", height="380px"))
                       ),
                       card(section_title("Comparación de modelos",
                                          "R², RMSE y MAPE en calibración y LOOCV para todos los modelos"),
                            plotlyOutput("model_comparison", height="380px"))
              ),
              ...
              tabPanel("Aplicación",
                       card(style_extra=paste0("border-left:4px solid ",ACCENT2,";",
                                               "background:linear-gradient(135deg,",ACCENT2,"10 0%,",COLOR_CARD," 55%);"),
                            ...
                            div(style="display:flex;align-items:center;gap:10px;margin-top:12px;",
                                tags$span("Modelo utilizado:",
                                          style=paste0("font-size:12px;color:",COLOR_MUTED,";font-weight:600;text-transform:uppercase;letter-spacing:0.06em;")),
                                uiOutput("app_model_badge_ui"))
                       ),
                       # Sub-tabs — Estaciones virtuales PRIMERO
                       tabsetPanel(id="app_tabs", selected="app_estmap",
                                   tabPanel("Estaciones virtuales", value="app_estmap",
                                            uiOutput("app_estmap_content")),
                                   tabPanel("Serie anual",
                                            uiOutput("app_anual_content")),
                                   tabPanel("Serie mensual",
                                            uiOutput("app_mensual_content")),
                                   tabPanel("Media móvil espacial",
                                            uiOutput("app_movil_content")),
                                   tabPanel("Ciclo multianual",
                                            uiOutput("app_ciclo_content")),
                                   tabPanel("Mapa SSC",
                                            uiOutput("app_mapa_content"))
                       ),
                       # Filtros comunes (ocultos en Mapa SSC)
                       conditionalPanel(
                         condition="input.app_tabs != 'Mapa SSC'",
                         card(
                           div(style="display:flex;gap:20px;flex-wrap:wrap;align-items:flex-end;",
                               div(style="width:220px;",
                                   selectInput("app_estaciones","Estaciones:",
                                               choices=c("Todas"="all"), selected="all", multiple=TRUE)),
                               div(style="min-width:260px;",
                                   sliderInput("app_window","Ventana media móvil espacial (nro estaciones):",
                                               min=2, max=6, value=3, step=1, width="100%")),
                               div(style="min-width:260px;",
                                   sliderInput("app_ma_window","Media móvil temporal (meses, 0=ninguna):",
                                               min=0, max=12, value=0, step=1, width="100%")),
                               div(
                                 radioButtons("app_show_calamar","Comparar con Calamar:",
                                              choices=c("Sí"="si","No"="no"), selected="si", inline=TRUE))
                           ),
                           sliderInput("app_year_slider","Intervalo de años:",
                                       min=2018, max=2026, value=c(2019,2026), step=1, sep="", width="100%")
                         )
                       ),
              ...
              tabPanel("Conclusiones",
                       card(style_extra=paste0("border-left:4px solid ",COLOR_ACCENT,";"),
                            ...
                            tags$p(
                              "El proceso de control de calidad permitió consolidar entre 27 y 39 observaciones de calibración
          distribuidas en los kilómetros 0, 1, 3, 14, 17, 18 y 19, alcanzando coeficientes de determinación
          entre 0.58 y 0.73. Las bandas Red, NIR y rojo 3 presentaron las correlaciones más altas con la
          concentración de sedimentos suspendidos (SSC), resultado consistente con lo reportado en la literatura
          para sistemas fluviales turbios. Entre los modelos evaluados, la regresión potencial log-log basada en
          la banda NIR mostró el mejor equilibrio entre simplicidad y desempeño, con métricas satisfactorias
          tanto en calibración como en validación LOOCV. Los modelos de aprendizaje automático como Random Forest
          y Gradient Boosting mostraron potencial, aunque el tamaño actual del dataset limita su capacidad de
          generalización.",
                              style=paste0("font-size:15px;color:",COLOR_TEXT,";line-height:1.9;margin-bottom:20px;text-align:justify;")
                            ),
                            tags$p(
                              "Con aproximadamente 30 observaciones, el conjunto de datos resulta adecuado para aplicar regresiones
          potenciales y modelos múltiples validados mediante LOOCV, mientras que una ampliación futura de la base
          de datos permitiría implementar enfoques de machine learning de manera más robusta. El modelo desarrollado
          busca ofrecer una alternativa frente a la limitada disponibilidad de datos in situ en el río Magdalena;
          sin embargo, esta misma limitación condiciona su alcance y capacidad predictiva. Además, actividades como
          los dragados en estaciones cercanas a la desembocadura pueden introducir ruido adicional en la señal
          espectral y deben considerarse durante la interpretación de los resultados y en futuros procesos de
          calibración.",
                              style=paste0("font-size:15px;color:",COLOR_TEXT,";line-height:1.9;margin-bottom:0;text-align:justify;")
                            )
                       ),
                       card(
